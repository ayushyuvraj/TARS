export type V2ActionType =
  | "NAVIGATE_STAGE"
  | "UPDATE_MAPPING"
  | "ADD_RULE"
  | "TOGGLE_RULE"
  | "RUN_RECONCILIATION"
  | "AUTO_RECONCILE_SUCCESS";

export interface V2Action {
  action: V2ActionType;
  payload: Record<string, any>;
  status?: string;
}

export interface V2WorkspaceContext {
  activeStage: string;
  stageNumber: number;
  stageLabel: string;
  sessionId: string | null;
  gstrFilename?: string;
  prFilename?: string;
  correlations?: any[];
  availableColumnsGstr?: string[];
  availableColumnsPr?: string[];
  selectedRuleIds?: string[];
  rulesSummary?: { id: string; name: string; isActive: boolean }[];
  selectedRecordId?: string | null;
  selectedRecordData?: Record<string, any> | null;
  resultsSummary?: { exact: number; tolerance: number; nearMatch: number; unresolved: number };
}

let currentContext: V2WorkspaceContext | null = null;
const contextListeners = new Set<(ctx: V2WorkspaceContext | null) => void>();
const actionHandlers = new Map<V2ActionType, Set<(payload: any) => void | Promise<void>>>();

export const copilotV2Bridge = {
  setContext(ctx: Partial<V2WorkspaceContext> | null) {
    if (ctx === null) {
      currentContext = null;
    } else {
      currentContext = {
        activeStage: ctx.activeStage || "setup",
        stageNumber: ctx.stageNumber ?? 1,
        stageLabel: ctx.stageLabel || "Setup & Dual Ingestion",
        sessionId: ctx.sessionId ?? null,
        ...ctx,
      };
    }
    contextListeners.forEach((fn) => fn(currentContext));
  },

  getContext(): V2WorkspaceContext | null {
    return currentContext;
  },

  subscribe(listener: (ctx: V2WorkspaceContext | null) => void): () => void {
    contextListeners.add(listener);
    listener(currentContext);
    return () => {
      contextListeners.delete(listener);
    };
  },

  registerActionHandler(
    type: V2ActionType,
    handler: (payload: any) => void | Promise<void>
  ): () => void {
    if (!actionHandlers.has(type)) {
      actionHandlers.set(type, new Set());
    }
    actionHandlers.get(type)!.add(handler);
    return () => {
      const handlers = actionHandlers.get(type);
      if (handlers) {
        handlers.delete(handler);
        if (handlers.size === 0) {
          actionHandlers.delete(type);
        }
      }
    };
  },

  dispatchAction(action: V2Action): boolean {
    const handlers = actionHandlers.get(action.action);
    if (handlers && handlers.size > 0) {
      let anySuccess = false;
      for (const handler of Array.from(handlers)) {
        try {
          void handler(action.payload);
          anySuccess = true;
        } catch (err) {
          console.error(`[CopilotV2Bridge] Action handler error for ${action.action}:`, err);
        }
      }
      return anySuccess;
    }
    console.warn(`[CopilotV2Bridge] No handlers registered for action: ${action.action}`);
    return false;
  },
};
